...
"""DATASETS"""
from multiframe_dataset import NormalizedMultiFrameDataset
from torch.utils.data import DataLoader
from performance_mesure import evaluate_reconstruction, plot_metrics_comparison,plot_comprehensive_comparison
from models.U_Net_5channels_inputsDROPOUT.norm_denorm import normalize_3d_tensor, denormalize_3d_tensor
...
def atualizar_coluna_losses(lista_losses, arquivo_csv="Untitled_losses"):
    ...
    with open(f'./losses/{arquivo_csv}.csv', 'w', newline='') as arquivo:
...
def load_sequences_from_tensor(csv_path, tensor_path, top_n,n_input_frames=5, times_aread=1, batch_size=32, seed=42):
    """
    ...
    with open("../../data/tensores/filtrados_sum_above5mm/timestamps_filtrado_5mm.csv", 'r') as arquivo:
...
model_name = f"5imagesUNET_predict{n_stamps_aread}aread_{n_sequencias_datasets}seq_batch{batchs_size}_ep{num_epochs}_pat{patience}_lr{str(learning_rate).replace('.', '_')}_drop{str(drop_rate).replace('.', '_')}_initScale{str(init_scale).replace('.', '_')}"
os.makedirs('saved_models', exist_ok=True)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Inicialização do modelo
model = UNetAutoencoder(dropout_rate=drop_rate, init_scale= init_scale).to(device)
criterion = nn.HuberLoss()
optimizer = optim.Adam(model.parameters(), lr=learning_rate)

# Carregamento dos dados
train_loader, val_loader, test_loader, test_dataset, normalization_max_val = load_sequences_from_tensor(
    csv_path=path_csv_seq,
...
for epoch in range(num_epochs):
    ...
    #Lógica force_quit
    try:
        with open('force_quit.txt', 'r') as f:
            force_quit = f.read().strip().lower() == 'true'
    except FileNotFoundError:
        force_quit = False

    if force_quit:
        print(f"FORCE_QUIT: Treinamento interrompido na época {epoch}")
        break
    ...
    atualizar_coluna_losses(train_losses, arquivo_csv=f"train_losses_{model_name}")
...
####FASE DE VALIDAÇÃO#######
    ...
    #salva csv de historico de losses
    atualizar_coluna_losses(val_losses, arquivo_csv=f"val_losses_{model_name}")
...
# Salva o gráfico na pasta com o nome do modelo
plot_filename = f"saved_models/{model_name}/training_validation_loss.png"
plt.savefig(plot_filename, bbox_inches='tight', dpi=300)
plt.show()
plt.close()

print(f"Gráfico de losses salvo em: {plot_filename}")
...
def evaluate_model(loader, model, denormalize=False, max_val=None):
    all_inputs = []  # Lista completa de 5 frames (para visualização)
    all_inputs_last = []  # Apenas último frame (para métricas de persistência)
    all_outputs = []
    all_targets = []
    ...
    for test_batch in loader:
        ...
        # Armazenar com índices e timestamps
        for i in range(len(input_indices)):
            # ⭐⭐ PARA VISUALIZAÇÃO: Lista completa de 5 frames
            input_frames_complete = []
            for j in range(input_images_np.shape[1]):  # Para cada timestep
                frame_idx = input_indices[i][j]  # Índice do frame
                frame_data = input_images_np[i, j]  # Dados do frame
                input_frames_complete.append((frame_idx, frame_data))

            # ⭐⭐ PARA MÉTRICAS: Apenas o último frame
            last_frame_idx = input_indices[i][-1]  # Último índice
            last_frame_data = input_images_np[i, -1]  # Último frame

            # Para target e prediction
            target_idx = target_index[i]
            pred_idx = target_idx

            # Armazenar ambos
            all_inputs.append(input_frames_complete)  # Lista de 5 frames
            all_inputs_last.append((last_frame_idx, last_frame_data))  # Último frame apenas
            all_outputs.append((pred_idx, test_outputs_np[i]))
            all_targets.append((target_idx, target_image_np[i]))

    return all_inputs, all_inputs_last, all_outputs, all_targets
# Avaliação
x_denorm,x_denorm_last, y_pred_denorm, y_true_denorm = evaluate_model(
...
# Plot comparativo
plot_metrics_comparison(predict_metrics_denorm, persist_metrics_denorm, "Unet5images", "Persistência", model_name_dir=model_name)

# Plot abrangente
plot_comprehensive_comparison(y_true_denorm, y_pred_denorm, x_denorm, timestamps, top_n=3, model_name=model_name)
